...
//         .mouseup  ((e) => this.MouseUp(e));
   ...
   CalcDragPixel(pP) {
      ...
      let {pointZ: pZ, pointA: pA, pointB: pB, pointC: pC, pointD: pD} = this;

      let pQ = this.pointQ = this.Intersection(pP, -this.theta1, pC, this.theta2);
      ...
      if (lCQ < this.radD) {
         let pT = this.pointT = this.Intersection(pP, -this.theta1, pD, this.theta2);
         let lDT = this.Distance(pD, pT);
         let lTU = lDT >= this.radD ? 0 : Math.sqrt(this.radD*this.radD - lDT*lDT);
         this.pointU = this.Move(pT, -this.theta1, lTU);
      } else {
      ...
      let pR  = this.pointR = this.Move(pQ, -this.theta1 + Math.PI, lQR);
      let lRU = this.Distance(pR, this.pointU);
      let lPR = this.Distance(pR, pP);
      let lQR2 = this.Distance(pR, pQ);
      this.pointS = this.Move(pQ, -this.theta1 + Math.PI, lQR - lPR * lQR2 / lRU);
   ...
   BoundingBox() {
      let {pointZ: pZ, pointC: pC, radC} = this;
